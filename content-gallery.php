...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<header class="entry-header">
			<h1 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'cute-frames' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h1>
		...
		<div class="entry-content">
			...
			<?php if ( $images ) : ?>
			<p><?php printf( _n( 'This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'cute-frames' ),
					'href="' . get_permalink() . '" title="' . esc_attr( sprintf( __( 'Permalink to %s', 'cute-frames' ), the_title_attribute( 'echo=0' ) ) ) . '" rel="bookmark"',
		...
		<footer class="entry-meta">
			<?php if ( 'post' == get_post_type() ) : // Hide category and tag text for pages on Search ?>
				<?php
					...
					$categories_list = get_the_category_list( __( ', ', 'cute-frames' ) );
					if ( $categories_list && cuteFrames_categorized_blog() ) :
				?>
				<span class="cat-links">
					<?php printf( __( 'Posted in %1$s', 'cute-frames' ), $categories_list ); ?>
				</span>
				<?php endif; // End if categories ?>

				<?php
					/* translators: used between list items, there is a space after the comma */
					$tags_list = get_the_tag_list( '', __( ', ', 'cute-frames' ) );
					if ( $tags_list ) :
				?>
				<span class="sep"> | </span>
				<span class="tag-links">
					<?php printf( __( 'Tagged %1$s', 'cute-frames' ), $tags_list ); ?>
				</span>
